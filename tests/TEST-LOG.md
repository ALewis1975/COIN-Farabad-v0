...
## 2026-06-09 — Spawn-pattern item (c): rooftop/tower + district_centroid placement (Issue #633)

- 2026-06-09T16:40Z | commit: 0ac32f7 | Mode: B | Scenario: Added `rooftop`/`tower` and `district_centroid` placement strategies to the overlay applier (`ARC_fnc_worldSpawnOverlayApply`). Rooftop/tower select the highest-`z` slot positions from the pre-scanned `ARC_worldBuildingSlots` registry (z preserved, not flattened); `indoor` now consumes real building slots; `district_centroid` reads the replicated `civsub_v1_districts` centroid with a small jitter ring. All paths use bounded radial fallbacks and a one-time `[ARC][SPAWNPAT][WARN]` (`ARC_overlayPlacementRegWarned`) when a registry is nil — never a new unbounded scan. Matrix wired: DEFEND `overwatch` + RAID `lookout` request rooftop; CIVIL + COMMUNITY_ENGAGEMENT `pedestrian` request district_centroid. | Steps: `bash tests/static/spawn_pattern_overlay_contract_checks.sh` (PASS — asserts rooftop/tower/district_centroid case labels exist, reuse `ARC_worldBuildingSlots`/`civsub_v1_districts`, no new `BIS_fnc_buildingPositions`, `nearestObjects [` count ≤1) + `python3 scripts/dev/sqflint_compat_scan.py --strict functions/world/fn_worldSpawnOverlayApply.sqf data/farabad_spawn_patterns.sqf` (PASS) | Result: PASS | Notes: Engine/JIP placement visibility = BLOCKED (no Arma runtime in sandbox); rooftop `z` uses building-position world z with `setPosATL`, to be verified on the dedicated rig before the toggle is enabled. Gated by `ARC_spawnPatternsEnabled`/`ARC_incidentOverlaySpawnsEnabled` (default OFF) — gameplay-neutral.

## 2026-06-09 — Spawn-pattern item (b): zone-sensitive checkpoint variants (Issue #633 step 6)

- 2026-06-09T16:45Z | commit: f2ac20f | Mode: B | Scenario: Added zone-differentiated checkpoint ambient flow. Matrix gains sibling overlays `CHECKPOINT_GATE` (controlled `gate_lane`, TNP/TNA guards, mixed pedestrians + civ vehicles), `CHECKPOINT_URBAN` (pedestrians/taxis/vendors, higher crowd density), and `CHECKPOINT_RURAL` (fewer pedestrians, trucks/pickups, `roadside` placement) alongside the default `CHECKPOINT`. `ARC_fnc_worldSpawnPatternResolve` takes a new 4th `_zone` STRING param and, for `CHECKPOINT`, maps AIRBASE/GREENZONE/MILITARYBASE→GATE, FARABADCITY→URBAN, else→RURAL, falling back to plain `CHECKPOINT` if a variant is absent; civic>lead>incident precedence preserved for all other types. Caller `fn_execInitActive.sqf` passes the already-computed `_zone` (default `""` keeps backward compat). | Steps: `bash tests/static/spawn_pattern_matrix_contract_checks.sh` (PASS — asserts all three `CHECKPOINT_*` keys exist with distinct overlays/placements and the resolver accepts/handles the zone arg + zone→variant mapping) + `python3 scripts/dev/sqflint_compat_scan.py --strict functions/world/fn_worldSpawnPatternResolve.sqf functions/core/fn_execInitActive.sqf data/farabad_spawn_patterns.sqf` (PASS) | Result: PASS | Notes: Engine/JIP ambient-flow demonstration (one urban/one gate/one rural checkpoint) = BLOCKED (no Arma runtime). Default-off toggles → gameplay-neutral.

## 2026-06-09 — Spawn-pattern item (a): building-purpose classification layer (Issue #633 step 5)

- 2026-06-09T16:50Z | commit: a354fe5 | Mode: B | Scenario: Added a per-building/per-slot purpose classification layer. New server-only `ARC_fnc_worldBuildingPurposeClassify` (registered in `CfgFunctions.hpp`, gated by `ARC_spawnPatternsEnabled`, first line `if (!isServer) exitWith {}`) consumes the pre-scanned `ARC_worldBuildingSlots` registry (no new scans) plus matrix tables (`locationPurposes`, `siteTypePurposes`, new `buildingClassPurposeHints`) and writes a server-local, non-broadcast `ARC_worldBuildingPurpose` registry (`locationId → purposeTag`) with classification order locationPurposes → name-hint override → nearest terrain site → NONE. Invoked once from `fn_worldInit.sqf` after `ARC_fnc_worldScanBuildingSlots`. `ARC_fnc_worldSpawnPatternAudit` extended to tally per-purpose counts, warn on NONE-classified locations, and emit `buildingPurposeCounts`. | Steps: `bash tests/static/building_purpose_classification_contract_checks.sh` (PASS — function registered, server-only, toggle-gated, reuses `ARC_worldBuildingSlots`, no `nearestObjects`/`BIS_fnc_buildingPositions`, writes non-broadcast registry, runs after the slot scan, every issue-listed purpose tag representable, hints resolve to real patterns) + `bash tests/static/spawn_pattern_matrix_contract_checks.sh` + `bash tests/static/spawn_pattern_overlay_contract_checks.sh` (PASS) + `python3 scripts/dev/sqflint_compat_scan.py --strict functions/world/fn_worldBuildingPurposeClassify.sqf functions/world/fn_worldSpawnPatternAudit.sqf functions/world/fn_worldInit.sqf` (PASS) | Result: PASS | Notes: New suite wired into `.github/workflows/arma-preflight.yml`. Engine/JIP classification + role-coherence = BLOCKED (no Arma runtime). Default-off toggle → gameplay-neutral.

## 2026-06-10 — Read-only architect audit remediation (Issue #642)

- 2026-06-10T00:35Z | commit: 6471641 | Mode: C/I | Scenario: Implemented the three P2 findings from the 2026-06-09 read-only architect/auditor report (Issue #642). (1) Namespace ownership: the five client-local once-per-client flags flagged in Finding 1 (`ARC_medAceIncapHandlerRegistered`, `ARC_uiTaskTimers_running`, `ARC_activeIncidentSitrepSentPending`, `ARC_activeIncidentSitrepSentPendingAt`, `ARC_console_keybindInit`) moved from `missionNamespace setVariable` (2-arg, local) to `uiNamespace setVariable`, with their paired `getVariable` reads updated in the same files — behavior parity per machine, no replication change. (2) CfgRemoteExec allowlist drift: removed the four verified-dead entries (`ARC_fnc_casreqDecide`, `ARC_fnc_casreqExecute`, `ARC_fnc_casreqClose`, `ARC_fnc_intelTocIssueLead`) — repo-wide grep confirms no `remoteExec`/`remoteExecCall` callsite for any of them (`intelTocIssueLead` only via local `call` in `fn_intelQueueDecide.sqf`); inline comments document the removal. (3) sqflint compat: replaced 3× `isNotEqualTo` and 2× `#` indexing in `fn_threatMarkCleanedByLabel.sqf` with `!(_a isEqualTo _b)` / `select`; also cleared the latent compat violations (3× `isNotEqualTo`, 1× direct `trim`) in `fn_uiTaskTimersInitClient.sqf` since Finding 1 touches that file and preflight scans changed files. | Steps: `python3 scripts/dev/sqflint_compat_scan.py --strict` on all 5 changed SQF files (PASS, no parser-compat patterns) + all `tests/static/*.sh` suites including `rpc_owner_capture_conformance_checks.sh` (ALL PASS) + `git --no-pager diff --check` (clean) | Result: PASS | Notes: No replicated-state semantics changed; each finding independently revertible. Dedicated-server/JIP smoke = BLOCKED (no Arma runtime in sandbox).

## 2026-06-10 — SitePop LAMBS function names (playtest RPT fix #1)

- 2026-06-10T15:50Z | commit: 4800e7e | Mode: A | Scenario: Fixed `ARC_fnc_sitePopApplyAmbiance` calling non-existent LAMBS functions. The 2026-06-09 VDS playtest RPT showed 6× `lambs_danger_fnc_garrison not found` and 6× `lambs_danger_fnc_camp not found` warnings despite LAMBS Danger (1858075458) being loaded — the garrison/camp waypoint functions live in the `lambs_wp` addon as `lambs_wp_fnc_taskGarrison`/`lambs_wp_fnc_taskCamp` (the names `fn_opsSpawnRouteSupport.sqf:685` and `fn_civsubInteractHandoffSheriff.sqf:248` already use). Every garrison/camp SitePop site silently fell back to SAFE/WHITE hold or plain loiter. Replaced both `isNil` guards, both `spawn` callsites, and the WARN messages; garrison call now mirrors the established repo arg pattern `[_grp, _p3, _radius, [], false, true, 3, false]` (exitCondition=3 firedNear). Vanilla fallbacks unchanged. | Steps: `python3 scripts/dev/sqflint_compat_scan.py --strict functions/sitepop/fn_sitePopApplyAmbiance.sqf` (PASS) + `sqflint -e w functions/sitepop/fn_sitePopApplyAmbiance.sqf` (exit 0) + `git --no-pager diff --check` (clean) | Result: PASS | Notes: Runtime verification (garrison groups occupying buildings at SitePop sites, no `not found` WARNs in RPT) = BLOCKED in sandbox; verify on the dedicated rig next playtest.

## 2026-06-10 — CASEVAC lead origin-position guard (playtest RPT fix #2)

- 2026-06-10T16:00Z | commit: 76b52b4 | Mode: A | Scenario: Fixed CASEVAC leads created with `grid=000000 zone=""` (2026-06-09 VDS playtest RPT: leads ARC_lead_9/10/11/54/55 for "Kyle Hall"/"Reed" at grid 000000). Root cause: a unit object can still be non-null at lead time while `getPosATL` returns a map-origin position (deleted unit / corpse parked at [0,0,0] by cleanup), which passed the old `count >= 2` check. `ARC_fnc_medicalCasevacRequest` now validates positions with a local `_isUsablePos` helper (array with numeric X/Y components AND ≥100 m from the map origin) at every step of the fallback chain: unit pos → `activeIncidentPos` → skip lead creation with a WARN naming the unit and rejected pos. Also hardened the `resize 3` z-pad (nil z → 0) so downstream marker/leadCreate consumers never see an Any z. | Steps: `python3 scripts/dev/sqflint_compat_scan.py --strict functions/medical/fn_medicalCasevacRequest.sqf` (PASS) + `sqflint -e w` (exit 0) + Python parity harness mirroring the validator (origin/near-origin/non-numeric/empty rejected; valid map and 2-element positions accepted; 100 m boundary correct) (PASS) + `git --no-pager diff --check` (clean) | Result: PASS | Notes: Cooldown/security/intel-log behavior unchanged; leads with a usable fallback position keep their real grid/zone. Runtime verification (no grid=000000 CASEVAC leads in next playtest RPT) = BLOCKED in sandbox; verify on the dedicated rig.


## 2026-06-10 — Idle-gating: pause lead gen, medical decay, ambient spawn ticks on empty server

- 2026-06-10T16:08Z | commit: d95b87d | Mode: B | Scenario: Added a server-only idle gate (`ARC_fnc_idleGateActive`, registered in `CfgFunctions.hpp`) that pauses background simulation when no interfaced human players are connected (headless clients excluded via `isPlayer` + `HeadlessClient_F` filter). Gated ticks: ambient lead/rumor/reactive-contact generation (`fn_civsubSchedulerTick.sqf`), ambient sustainment decay incl. medical (`fn_supplyApplyAmbientDrain.sqf` — idle exit advances `sustainLastAt`/`supply_v1_last_ambient_tick` so the empty interval never accrues retroactive drain), medical recovery clock (`fn_medicalTick.sqf`, frozen for symmetry so baseMed neither decays nor recovers while empty), and the ambient spawn ticks (`fn_civsubCivSamplerTick.sqf`, `fn_civsubTrafficTick.sqf`, `fn_civsubLocNpcTick.sqf`, `fn_airbaseGroundTrafficTick.sqf`, `fn_sitePopTick.sqf`). Helper is per-frame cached (diag_frameNo, same pattern as `ARC_fnc_playerSnapshot`), logs `[ARC][IDLE][INFO]` once per pause/resume transition, and only engages after a grace window (`ARC_idleGateGraceS`, default 300 s — deliberately above the sitepop despawn grace so proximity cleanup/caps enforcement winds ambience down before the pause). Master disable toggle `ARC_idleGateEnabled` (default true) seeded in `initServer.sqf`. | Steps: `bash tests/static/idle_gate_contract_checks.sh` (PASS — 19/19: helper server-only/disable-toggle/HC-exclusion/grace/frame-cache/logging, CfgFunctions registration, initServer seeds, all 8 gated callsites, drain-clock reset) + `python3 scripts/dev/sqflint_compat_scan.py --strict` on all 9 changed SQF files (PASS) + `sqflint -e w` on all 9 changed SQF files (exit 0) + `git --no-pager diff --check` (clean) | Result: PASS | Notes: New static suite wired into `.github/workflows/arma-preflight.yml`. Runtime verification (pause/resume RPT transitions on player disconnect/reconnect, no retroactive MED drain after an idle window, ambience respawn on reconnect, JIP behavior) = BLOCKED in sandbox; verify on the dedicated rig.

## 2026-06-10 — Fix: Farabad Console keybind (Ctrl+Shift+T) dead after mission restart/rejoin

- 2026-06-10T21:35Z | commit: 8e88982 | Mode: A | Scenario: Root cause for "lost the ability to access the Farabad Console via L-CTRL+L-SHIFT+T": commit 6471641 (issue #642 audit) moved once-per-client init guards from `missionNamespace` to `uiNamespace`, but `uiNamespace` variables persist for the entire Arma game session — across mission restarts and server rejoins — while CBA keybind handlers, the task-timer HUD layer/loop, and CBA event handlers are mission-scoped. After the first mission of a client game session the stale `true` guard short-circuited `ARC_fnc_uiConsoleInitClient`, so `CBA_fnc_addKeybind` was never re-called and the console keybind went dead. Fix: keep guards in `uiNamespace` (per namespace-ownership doctrine) but store a per-mission token (`format ["%1|%2", missionName, missionStart]`) instead of a boolean, so each new mission session re-registers. Applied identically to the three once-per-client guards migrated by 6471641: `ARC_console_keybindInit` (fn_uiConsoleInitClient.sqf), `ARC_uiTaskTimers_running` (fn_uiTaskTimersInitClient.sqf), `ARC_medAceIncapHandlerRegistered` (fn_medicalAceIncapHandler.sqf). Same-mission respawn/JIP re-entry still exits early (token matches), and CBA dedupes repeat addKeybind calls anyway. | Steps: `python3 scripts/dev/sqflint_compat_scan.py --strict` on the 3 changed SQF files (PASS) + `bash tests/static/console_field_request_relocation_checks.sh` (PASS) + `bash scripts/dev/check_console_conflicts.sh` (PASS) + `git --no-pager diff --check` (clean); sqflint binary unavailable in sandbox (runs in arma-preflight CI) | Result: PASS (static) | Notes: Runtime verification = BLOCKED in sandbox; on the dedicated rig: connect, confirm RPT `[ARC][CONSOLE] Keybind registered`, open console with Ctrl+Shift+T, then restart mission WITHOUT restarting the client game and confirm the keybind re-registers and still opens the console.

## 2026-06-10 — Broadcast-path perf fixes: district marker + Console VM change detection

- 2026-06-10T23:55Z | commit: fc0677d | Mode: D | Scenario: Implemented the two pending broadcast-path perf hotspots from the full performance review of the 2026-06-09 VDS playtest RPT (`VDSReports/ArmA3Server_x64_2026-06-09_17-37-04.rpt`: 2,939 `districtMarkersUpdate: updated 20 district influence markers` lines — the broadcast path runs every ~5 s via the airbase tick). (1) `fn_districtMarkersUpdate.sqf`: previously re-sent 6 global `setMarker*` commands for all 20 districts on every broadcast (~120 per-client net messages / 5 s, constant RPT spam). Now keeps a server-local signature cache (`ARC_districtMarkersApplied`: districtId → [pos, color, radius, text]) and skips all marker commands when the visuals are unchanged; markers that vanish (`in allMapMarkers` false) are recreated regardless of cache. (2) `fn_publicBroadcastState.sqf`: `ARC_consoleVM_payload` (full 15-section VM) and `ARC_consoleVM_meta` were re-replicated to every client each cycle with an unconditional rev bump even when no data changed. Now a data-only signature (section name + data pairs; rev/builtAtServerTime/freshness timestamps excluded) is compared against the last published signature, and payload+meta replication (and the rev bump) only occur on actual change or after a bounded max-age refresh (`ARC_consoleVmMaxPublishAgeS`, default 60 s, clamped 5..600) so embedded freshness timestamps cannot drift indefinitely. Constant `ARC_pub_stateSchema` is now replicated only when missing/outdated instead of every cycle. `ARC_pub_state` itself already had change detection via `ARC_fnc_statePublishPublic` (untouched). | Steps: `python3 scripts/dev/sqflint_compat_scan.py --strict functions/core/fn_districtMarkersUpdate.sqf functions/core/fn_publicBroadcastState.sqf` (PASS) + `sqflint -e w` on both files (exit 0) + static suites touching these files: `threat_ui_snapshot`, `threat_persistence_migration`, `threat_virtual_opfor_observability`, `lane_c`, `threat_economy_operator_tooling`, `airbase_queue_lifecycle`, `casreq_snapshot`, `console_vm_dashboard_migration` contract checks (ALL PASS) + `git --no-pager diff --check` (clean) | Result: PASS (static) | Notes: Client refresh signals are unaffected — PV event handlers key off `ARC_pub_stateUpdatedAt` etc. (still published on real change), the console keeps its 3 s fallback repaint, and the dashboard reads `ARC_consoleVM_payload` whose last replicated value remains valid for JIP. Runtime verification = BLOCKED in sandbox; on the dedicated rig confirm: RPT no longer fills with per-5s districtMarkersUpdate lines (only on influence change), console dashboard/S2 data still refreshes on state change, and JIP clients receive markers + VM payload.

## 2026-06-11 — Playtest RPT fixes: civsub no-district warn spam + threat backfill denials (2026-06-10 VDS review)

- 2026-06-11T16:45Z | commit: 1d50dd4 | Mode: A | Scenario: Two fixes from the 2026-06-10 VDS playtest RPT review (`VDSReports/ArmA3Server_x64_2026-06-10_19-32-49.rpt`). (1) `ARC_fnc_civsubCivConnect` emitted 853 `[CIVSUB][WARN] ... reason=no district` lines because the same unconnectable unit is retried from multiple sources (CIVLOC + ENTITY_CREATED + PERIODIC_SCAN, and the periodic scan re-warns every pass). Now warn-once per unit via a server-local (non-broadcast) `civsub_v1_noDistrictWarned` unit variable; later skips for the same unit are silent and still return false. (2) `ARC_fnc_execObjectiveComplete` attempted `CREATED→DISCOVERED`/`CREATED→NEUTRALIZED` on never-activated threat records (RPT: THR:D05:000240, 3× `DENY_TRANSITION_INVALID` WARNs + 3 denied events), but the state table in `fn_threatUpdateState.sqf` only permits `CREATED→ACTIVE/STAGED/CLOSED/EXPIRED`. Both the DISCOVER and COMPLETE stages now step a CREATED record through a legal `ACTIVE` backfill first (notes `OBJECTIVE_DISCOVERED_BACKFILL_ACTIVE` / `OBJECTIVE_COMPLETE_BACKFILL_ACTIVE`), re-read the state, and only then drive DISCOVERED → NEUTRALIZED; each subsequent transition is gated on the actual current state instead of the stale pre-read. ACTIVE has no side effects in `fn_threatUpdateState.sqf`; DISCOVERED/NEUTRALIZED side effects (lead emission, COIN influence) now fire as the lifecycle intends instead of being lost to denials. | Steps: `python3 scripts/dev/sqflint_compat_scan.py --strict functions/civsub/fn_civsubCivConnect.sqf functions/core/fn_execObjectiveComplete.sqf` (PASS) + ALL `tests/static/*.sh` suites (ALL PASS) + `git --no-pager diff --check` (clean); sqflint binary unavailable in sandbox (runs in arma-preflight CI) | Result: PASS (static) | Notes: Runtime verification = BLOCKED in sandbox; on the dedicated rig next playtest confirm (a) at most one no-district WARN per out-of-district unit, (b) no `DENY_TRANSITION_INVALID` WARNs on IED objective completion — expect `CREATED→ACTIVE→DISCOVERED→NEUTRALIZED` in the OPS log instead.

## 2026-06-29 — Bug fix: TABLET_FRAME z-order (issue #660)

- 2026-06-29T01:35Z | commit: ef37b9b (pre-commit; see PR for final SHA) | Mode: A | Scenario: Three changes from issue #660. (1) `config/CfgDialogs.hpp`: added `import RscPicture` and a static `TabletFrame: RscPicture` control (IDC 78141, `show=0`) as the first entry in `ARC_FarabadConsoleDialog.controlsBackground` so the tablet frame image is in the background controls layer and cannot draw above console content controls. (2) `functions/ui/fn_uiConsoleApplyLayout.sqf`: removed the `ctrlCreate ["RscPicture", 78141]` runtime-creation fallback; the function now only does a `displayCtrl 78141` lookup and calls `ctrlShow`/`ctrlSetPosition`. No `ctrlSetText` needed (text fixed in config). (3) `scripts/dev/check_console_conflicts.sh`: added `7814[1-9]` to the known-range regex and updated the range-audit legend so IDC 78141 no longer triggers an UNKNOWN warning. `fn_uiConsoleRefresh.sqf` Region C path was already correct (`_layoutModeActive in ["DOCK_RIGHT", "TABLET_FRAME"]`) — no change needed there. | Steps: `python3 scripts/dev/sqflint_compat_scan.py --strict functions/ui/fn_uiConsoleApplyLayout.sqf functions/ui/fn_uiConsoleRefresh.sqf` (PASS) + `bash scripts/dev/check_console_conflicts.sh` (PASS — no duplicate IDCs, all IDCs in documented ranges) + `git --no-pager diff --check` (clean) | Result: PASS (static) | Notes: Runtime verification = BLOCKED in sandbox; on the dedicated rig: set `ARC_console_layoutMode="TABLET_FRAME"`, open console, confirm all panes visible immediately without focus-change flicker, confirm frame image renders behind tabs/lists/buttons; switch to `FULL` and confirm tablet frame hides. Also confirm AIR tab map uses Region C in TABLET_FRAME and does not crowd list/detail panes.

## 2026-06-29 — Feature: ATC ramp queue + METT-TC priority + RAMP console submode

- 2026-06-29T02:48Z | commit: 8fbd66c | Mode: B | Scenario: Added two capabilities to the Airbase subsystem. (1) **Player ATC ramp queue**: new server RPC `ARC_fnc_airbaseRequestQueueParkedAsset` (APPROVE-tier authority, full RPC security chain: remoteExecutedOwner capture at top frame → rpcValidateSender → airbaseTowerAuthorize "APPROVE") reads the `airbase_v1_rt` HashMap to validate an asset is PARKED, allocates a flight ID from `airbase_v1_seq`, builds a departure record with `source=MANUAL_ATC`/`manualOverride=true`, validates route via `airbaseBuildRouteDecision`, then commits to `airbase_v1_queue` + `airbase_v1_records` via stateSet. Client wrapper `ARC_fnc_airbaseClientQueueParkedAsset` is a thin `remoteExec` shim (`hasInterface` guard). Both registered in `CfgFunctions.hpp`; server RPC allowlisted in `CfgRemoteExec.hpp` with `allowedTargets=2`. (2) **METT-TC AI priority selection**: `fn_airbaseTick.sqf` now reads `airbase_v1_assetDeparturePriorityOrder` (default: UAS/EW/Refuel/A-10/F-16/AH-64/UH-60/CH-47/C-17/C-130 prefix order) and walks the candidate pool to pick the highest-priority available parked asset; falls back to `selectRandom` when no candidate matches any prefix. (3) **RAMP console submode**: `fn_publicBroadcastState.sqf` builds `_uiParkedAssets` (PARKED assets not already in DEP queue) from the live `airbase_v1_rt` HashMap using a compiled `_fnHmGetBcast` helper, includes it in `ARC_pub_airbaseUiSnapshot` as `["parkedAssets", _uiParkedAssets]`, and adds `count _uiParkedAssets` to the change-detection key. `fn_uiConsoleAirPaint.sqf`: new RAMP submode surfaced in `_cycleModes` (after CLEARANCES, before DEBUG), valid-submode guard, list switch (asset rows with `ASSET|<id>|<cat>|<vehType>|<towFlag>` lbData, RAMP_HDR guidance row), button-labels switch (`QUEUE DEPARTURE` primary), and detail panel (`ASSET` and `RAMP_HDR` cases). `fn_uiConsoleActionAirPrimary.sqf`: `case "RAMP"` dispatches `ASSET` rows to `ARC_fnc_airbaseClientQueueParkedAsset`. `fn_uiConsoleActionAirSecondary.sqf`: RAMP added to `_cycleModes` and valid-submode guard; `case "RAMP"` secondary cycles view. | Steps: `bash tests/static/airbase_ramp_queue_contract_checks.sh` (PASS — 47/47 checks) + `git --no-pager diff --check` (clean) | Result: PASS (static) | Notes: Runtime verification = BLOCKED in sandbox. On the dedicated rig verify: (a) CCIC/BnCmd ATC can open RAMP submode via secondary button, see all PARKED assets not in DEP queue, press QUEUE DEPARTURE to send one, confirm it appears in departure queue in CLEARANCES view; (b) LC-tier account cannot see RAMP (falls back to AIRFIELD_OPS); (c) RPT shows `[ARC][AIRBASE][RAMP][INFO] queued fid=FLT-N aid=...`; (d) ambient scheduler now selects UAS/EW assets first when available (confirm via RPT departure OPS log); (e) `ARC_pub_airbaseUiSnapshot` parkedAssets count reflects the live ramp state after an asset departs.

## 2026-06-29 — Feature: ATC/Pilot coordination loop (Items 1–3)

- 2026-06-29T03:30Z | commit: c124a0a | Mode: B | Scenario: Three minimum-viable improvements to close the player ATC↔pilot coordination loop. **Item 1 — Departure runway reservation**: `fn_airbaseRequestClearanceDecision.sqf`: on REQ_TAKEOFF approval (inside `if (_approve) then {}` after all exitWith guards pass), calls `ARC_fnc_airbaseRunwayLockReserve` with `_requestId` as owner token, kind `"DEP"`, reason `"PLAYER_APPROVAL"`, window `airbase_v1_player_dep_reserve_s` (default 300 s). After the existing approval toast to ATC, sends "Runway reserved. Taxi to runway and depart when ready." via `ARC_fnc_clientHint remoteExec` to the pilot's machine owner (`_requesterOwner`), gated on DEP flow types. **Item 2 — Arrival notifications**: `fn_airbaseTick.sqf`: (a) After the `LIFECYCLE_LAND_GATE` event push (auto-land gate at 2200 m, REQ_INBOUND→REQ_LAND), loops `_towerControllers` sending a "LAND GATE: <pilot> on final (<dist>m) — issue landing clearance" TOAST via `_fnNotifyMaybe` with dedupeKey `AIR_LAND_GATE:<rid>`. (b) After the `STALE_INBOUND_NEAR_RUNWAY` escalation event push, loops tower controllers with "URGENT: <pilot> uncleared at <dist>m — decision required now" (dedupeKey `AIR_STALE_INB:<rid>`) and sends the inbound pilot "Your inbound <rid> is URGENT — awaiting tower clearance" (dedupeKey `AIR_STALE_PILOT:<rid>`). **Item 3 — CLEARANCES UI enhancements**: `fn_uiConsoleAirPaint.sqf`: (a) Runway status row added at the top of the CLEARANCES submode header area (before the pending clearances header row), reading `_runwayState`, `_runwayOwnerDisplay`, `_holdDepartures`, and `_activeMovement` from the already-parsed snapshot, using `_rwyOwnerTagC`/`_rwyHoldLabelC` locals to avoid name collision with AIRFIELD_OPS block; lbData tag `"RWY|..."` routes clicks to the existing RWY detail panel. (b) Distance/warning tag appended to clearance row labels for `REQ_INBOUND`/`REQ_LAND` types, reading `arrivalDistanceM` and `arrivalWarnLevel` from the meta tuple at index 7. (c) REQ detail panel: arrival distance line with color coding (URGENT=#E74C3C, CAUTION=#F5A623, default white) added before the action buttons. Static contract suite `tests/static/airbase_queue_lifecycle_contract_checks.sh` extended with §6 (4 checks), §7 (7 checks), §8 (6 checks). Pre-existing sqflint warning on `_resolveAircraftDisplay` in `fn_uiConsoleAirPaint.sqf` is unchanged (scope warning in RAMP submode, not introduced by this PR). | Steps: `python3 scripts/dev/sqflint_compat_scan.py --strict functions/ambiance/fn_airbaseRequestClearanceDecision.sqf functions/ambiance/fn_airbaseTick.sqf functions/ui/fn_uiConsoleAirPaint.sqf` (PASS) + `sqflint -e w` on all 3 changed SQF files (PASS — 0 new warnings introduced) + `bash tests/static/airbase_queue_lifecycle_contract_checks.sh` (PASS — 76/76) + `bash tests/static/airbase_planning_mode_checks.sh` (PASS) + `bash tests/static/airbase_ramp_queue_contract_checks.sh` (PASS — 47/47) + `git --no-pager diff --check` (clean) | Result: PASS (static) | Notes: Runtime verification = BLOCKED in sandbox. On the dedicated rig verify: (a) ATC approves REQ_TAKEOFF → RPT logs `[ARC][AIRBASE] airbaseRunwayLockReserve` + runwayState flips RESERVED, pilot receives "Runway reserved" hint; (b) inbound at 2200 m → ATC tab shows "LAND GATE" toast, REQ row shows distance/warn tag; (c) stale inbound near runway → ATC receives URGENT toast, pilot receives "awaiting tower clearance" hint; (d) CLEARANCES submode shows runway state row at top; (e) runway lock sweeps back to OPEN after `airbase_v1_player_dep_reserve_s` if aircraft never occupies runway; (f) repeat stale-inbound URGENT toasts fire at most every `airbase_v1_notifyThrottle_s` (~8 s) per request ID.

## 2026-07-02 — Fix: bare-createHashMapFromArray compat + PR #687 alias cleanup

- 2026-07-02T00:21Z | commit: df3035a | Mode: A | Scenario: CI preflight failure fix — `data/farabad_marker_aliases.sqf` used bare `createHashMapFromArray` which sqflint 0.3.2 cannot parse (rule: `bare-createHashMapFromArray`). Applied PR #687 alias cleanup (remove stale marker_12/13/15/17/22/ARC_m_logistics_01 aliases; add marker_14→ARC_loc_OilProcessing, marker_16→ARC_loc_Mine16, marker_18-21→ARC_loc_Mine18-21; expand comment block) + wrapped `createHashMapFromArray` in a compiled helper (`private _hmCreate = compile "params ['_a']; createHashMapFromArray _a"; [...] call _hmCreate`) to satisfy compat scanner. Also applied matching PR #687 data changes: added OilProcessing/Mine16/Mine18-21 to `data/farabad_world_locations.sqf`; replaced raw marker_14/16/18-21 with canonical ARC_loc_* names in `data/incident_markers.sqf`. | Steps: `python3 scripts/dev/sqflint_compat_scan.py --strict data/farabad_marker_aliases.sqf data/farabad_world_locations.sqf data/incident_markers.sqf` (PASS — 0 findings) + `sqflint -e w data/farabad_marker_aliases.sqf` (PASS) + `sqflint -e w data/incident_markers.sqf` (PASS) | Result: PASS (static) | Notes: Runtime verification = BLOCKED in sandbox. On the dedicated rig verify: (a) no RPT compile errors from the three data files; (b) ARC_loc_OilProcessing, ARC_loc_Mine16, ARC_loc_Mine18-21, ARC_loc_military all created after world init; (c) incidents referencing new ARC_loc_* markers resolve correctly and are not skipped; (d) ARC_m_logistics_01 still resolves as its own static logistics marker.
