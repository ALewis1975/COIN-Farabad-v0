...
- 2026-02-17T19:08Z | commit: <pending> | Scenario: follow-up static check for legacy intel compatibility fix (`sqflint -e w functions/core/fn_taskBuildDescription.sqf`) | Result: BLOCKED | Notes: `sqflint` is unavailable in this container (`command not found`), so lint verification could not run.
- 2026-02-17T19:08Z | commit: <pending> | Scenario: follow-up patch sanity (`git diff --check`) | Result: PASS | Notes: No whitespace or patch-format issues after legacy-entry guard adjustment.
