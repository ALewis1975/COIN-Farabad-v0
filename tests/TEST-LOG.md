...
**Branch/Commit:** copilot/fix-civsub-caps-enforce-error @ 304130b
...
| 8 | CI follow-up investigation | GitHub Actions run `76069017222`; local file review of `.github/workflows/arma-preflight.yml` SQF static analysis step | FAIL | User-provided CI logs identified sqflint parser failures on direct HashMap `get` expressions in the same changed SQF file. GitHub MCP lookup returned 404 in this sandbox. |
| 9 | Direct HashMap `get` scan | `grep -En '(^\|[^A-Za-z0-9_])get[[:space:]]+_[A-Za-z0-9_]+\|#\|getOrDefault \\[\|keys _' functions/civsub/fn_civsubCivCapsEnforce.sqf \|\| true` | PASS | Only compile-helper strings matched; no direct HashMap infix `get`, `#` indexing, bare `getOrDefault`, or bare `keys _map` remain outside compile-helper strings. |
