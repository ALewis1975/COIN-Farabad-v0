...
## 2026-06-29 — UAS screen: dynamic vehicle-type labels + helmScreen subsystem (PR follow-up to #659)

- 2026-06-29T03:30Z | commit: aab253e (pre-commit; see PR for final SHA) | Mode: B | Scenario: Two post-merge enhancements requested after #659 playtest. (1) **Dynamic vehicle-type labels** — UAS screen "Next/Previous" action titles now show the actual vehicle type or var-name of the next/previous UAV in the rotation (e.g., "[UAS] Next: Global Hawk RQ-4A") rather than the static generic string. New `ARC_fnc_uasScreenShortLabel` (common) returns vehicleVarName or CfgVehicles displayName. New `ARC_fnc_uasScreenRefreshActionTitles` (clientInit) calls `setUserActionText` after each selection or action rebind. `addAction` calls now capture individual IDs (`ARC_uasScreen_nextActionId`, `ARC_uasScreen_prevActionId`) instead of a diff-set. (2) **helmScreen subsystem** — new `scripts/helmScreen/` (common + serverInit + clientInit) mirrors the uasScreen subsystem on the same managed screen pool. Adds `[HELM] Next/Previous Helmet Cam` actions (cycle through friendly players carrying `ItemcTabHCam` by configurable `ARC_helmScreenHelmCamItem`; POV PiP attached to unit head/torso), `[HELM] Clear Screen Feed`, `[MAP] Show ISR Map View` (nadir PiP camera at configurable altitude above `ARC_activeIncidentPos` or map centre, simulating a cTab tactical overview), and `[MAP] Clear ISR Map View`. Server owns `ARC_helmScreenFeedSnapshot`; clients render via JIP-safe snapshot-watcher pattern identical to uasScreen. Two new CfgRemoteExec entries: `ARC_fnc_helmScreenRequestFeed`, `ARC_fnc_helmScreenRequestClear`. `hasItem` wrapped in compile helper for sqflint compat. | Steps: `python3 scripts/dev/sqflint_compat_scan.py --strict` on all 5 changed/new SQF files (PASS) + `sqflint -e w` on all 5 files (all PASS, exit 0) + `git --no-pager diff --check` (clean) | Result: PASS (static) | Notes: Runtime verification = BLOCKED in sandbox; on the dedicated rig: (a) confirm "Next/Previous" action labels update to show vehicle type after first press; (b) confirm helm-cam PiP appears on managed screen when a friendly player with ItemcTabHCam is present; (c) confirm ISR Map View renders a nadir overhead feed above the active AO; (d) confirm JIP client reconstructs active feeds from snapshot.
