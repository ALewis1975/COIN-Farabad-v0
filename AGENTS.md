...
- **Run on the dedicated server (rig now available — gated):** persistence durability across restarts, JIP snapshot correctness, late-client recovery for in-flight events, and reconnect/respawn ownership edge cases. A dedicated Armahosts VPS is now available (see `docs/architecture/Dedicated_Server_Activation_Plan_2026-05-27.md`); however, Track 4 validation (walking the validation matrix) is **gated on Track 1 + Track 2 landing** — do not execute or record these validations as definitive until both tracks are confirmed complete and the canonical modline is populated in `docs/operations/Server_ModLine.md` (any modline change invalidates prior PASS results). Once the gate is satisfied, execute these validations on the dedicated rig and record the results in `tests/TEST-LOG.md`. The container/CI sandbox still cannot run the Arma 3 runtime, so these checks remain `BLOCKED` for agent/CI runs and must be completed by an operator on the dedicated rig.
