...
| **F2** | P1 | `toUpperANSI`/`toLowerANSI` in 4 airbase files — CI sqflint rejects | `fn_airbaseRecordSetQueuedStatus.sqf`, `fn_airbaseRequestSetLaneStaffing.sqf`, `fn_airbaseBuildRouteDecision.sqf`, `fn_airbaseClearanceSortRequests.sqf` | Replace with `toUpper`/`toLower` |
| **F3** | P1 | `findIf` in 4 threat files — CI sqflint rejects | `fn_threatCreateFromTask.sqf`, `fn_threatDebugSnapshot.sqf`, `fn_threatGetCleanupLabelForTask.sqf`, `fn_threatOnIncidentClosed.sqf` | Replace with `forEach` + `exitWith` |
| **F4** | P1 | Bare `createHashMapFromArray` in identityTouch — sqflint rejects | `fn_civsubIdentityTouch.sqf:33` | Wrap in `_hmCreate` compile helper |
...
| Replace `toUpperANSI`/`toLowerANSI` in airbase files | ~12 replacements | 4 files |
| Replace `findIf` in threat files | ~10 replacements | 4 files |
| Wrap `createHashMapFromArray` in identityTouch | 3 lines | 1 file |
...
| 2 | **P1** | `toUpperANSI`/`toLowerANSI` → CI failure | Replace with `toUpper`/`toLower` | 12 |
| 3 | **P1** | `findIf` → CI failure | Replace with `forEach` + `exitWith` | 20 |
| 4 | **P1** | Bare `createHashMapFromArray` → CI failure | Wrap in compile helper | 3 |
