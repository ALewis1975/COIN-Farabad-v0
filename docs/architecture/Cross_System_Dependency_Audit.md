...
- Time/terrain/persistence facts computed independently by consumers instead of resolved through owner layers.
...
| `RUNTIME_BUDGET_RISK` | Scheduler/spawn path does not clearly consume runtime policy or cap discipline. | Add L0 review before new behavior. |
...
| DEP-007 | Government / GREEN | GREEN legitimacy exists in CIVSUB, but government agency behavior is not yet a narrow runtime contract. | `LEGACY_ACCEPTED` / `PLANNING` | CIVSUB / future Gov bridge | Write Government Layer planning spec before persistent government state or scheduler. |
| DEP-008 | OPFOR capacity | Threat currently carries pressure that may later belong to OPFOR network capacity. | `LEGACY_ACCEPTED` / `PLANNING` | Threat / future OPFOR bridge | Write OPFOR Network planning spec before persistent hidden network state. |
...
- [ ] Does the feature add scheduler or spawn pressure that requires L0 review?
...
Any new scheduler, spawn source, or physical ambience expansion must answer:
...
4. Every new scheduler/spawn path has L0 runtime-budget review.
...
- A scheduler/spawn path changes runtime pressure.
