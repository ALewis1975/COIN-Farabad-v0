...
if !(_gid isEqualType '' && { _gid isNotEqualTo '' }) exitWith {false};
...
private _hasApproval = {
    ...
    private _ok = false;
    {
        ...
        if ((toUpper (trim (_x # 2))) isNotEqualTo _reqType) then { continue; };
        private _exp = _x # 5;
...
if (_collected && _rtbApproved && !_delivered) then
{
    ...
    if (!_transportEnabled) then
    {
        ...
        if (_mode isEqualTo 'ACE_CARGO' && { _evNid isNotEqualTo '' }) then
        {
...
if (_collected && _rtbApproved && !_delivered) then
{
    ...
    else
    {
        ...
        private _near = false;
        {
            ...
            if ((groupId (group _x)) isNotEqualTo _gid) then { continue; };
