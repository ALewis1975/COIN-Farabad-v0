...
private _trimRxText = {
    params ["_v", ["_fallback", ""]];
    private _s = if (_v isEqualType "") then { [_v] call (compile "params ['_s']; trim _s") } else { _fallback };
...
_owner = toUpper ([_owner] call (compile "params ['_s']; trim _s"));
if (_owner != "INTEL") then { _rebuild = true; };
...
private _renderS2CatPanelsFromMaster = {
    ...
    for "_i" from 0 to ((lbSize _listMaster) - 1) do {
        ...
        if (_d in ["HDR", "SEP"]) then {
            private _sectionCandidate = toUpper ([_t] call (compile "params ['_s']; trim _s"));
...
private _storeComboSel = {
    ...
    if (([_d] call (compile "params ['_s']; trim _s")) isEqualTo "") then { _d = _defData; };
...
private _restoreComboSel = {
    ...
    _want = [_want] call (compile "params ['_s']; trim _s");
...
private _appendCivsubResult = {
    ...
    _typeExpect = toUpper ([_typeExpect] call (compile "params ['_s']; trim _s"));
...
else
{
    ...
    switch (_kind) do
    {
        ...
        case "CIV_CENSUS_DID":
        {
            private _did = [_arg] call (compile "params ['_s']; trim _s");
...
else
{
    ...
    switch (_kind) do
    {
        ...
        case "CIV_CONTACT_QUESTION":
        {
            ...
            if (!isNull _b1) then { _b1 ctrlEnable (_inCivCtx && {_qid != ""}); _b1 ctrlSetText "ASK"; };
        };
        ...
        case "OPFOR_STATUS":
        {
            ...
            private _opforEntries = [];
            {
                ...
                private _cat = if ((count _x) > 2) then { toUpper ([(_x select 2)] call (compile "params ['_s']; trim _s")) } else { "" };
            ...
            if ((count _opforEntries) > 0) then {
                _txt = _txt + "<t size='1.0' font='PuristaMedium' color='#B89B6B'>Recent Intel (SIGHTING/THREAT/ISR)</t><br/><br/>";
                {
                    ...
                    private _cat2  = toUpper ([(_x select 2)] call (compile "params ['_s']; trim _s"));
                    private _summ  = [(_x select 3)] call (compile "params ['_s']; trim _s");
                    if ((count _summ) > 80) then { _summ = (_summ select [0, 80]) + "..."; };
                    private _grid2 = if ((count _x) > 4) then {
                        private _iPos = _x select 4;
