    ...
    case "INCIDENT":
    {
        ...
        if (!isNull _b1) then
        {
            if (!_accepted) then
            {
                ...
                _b1 ctrlEnable (([player] call ARC_fnc_rolesIsAuthorized) || { ([(missionNamespace getVariable ["ARC_consoleOmniTokens", ["OMNI"]]), { [player, _x] call ARC_fnc_rolesHasGroupIdToken }] call _findIfFn) >= 0 });
            }
            else
            {
