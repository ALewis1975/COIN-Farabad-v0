...
private _latentTtl = missionNamespace getVariable ["ARC_threatLatentTtlS", 3600];
if !(_latentTtl isEqualType 0) then { _latentTtl = 3600; };
_latentTtl = (_latentTtl max 300) min 21600;
_rec = [_rec, "lifecycle_v", 1] call _kvSet;
_rec = [_rec, "latent_expires_ts", _now + _latentTtl] call _kvSet;
