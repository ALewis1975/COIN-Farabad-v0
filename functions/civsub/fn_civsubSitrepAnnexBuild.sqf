...
if (_start isEqualType []) then
{
    ...
    _startHm = [_start] call _hmFrom;
};

private _hasWindow = (_incDid isEqualTo _districtId) && { _startHm isEqualType createHashMap } && { (count ([_startHm] call _keysFn)) > 0 };
...
if (_last isEqualType []) then { _last = [_last] call _hmFrom; };
private _lastEvent = "";
