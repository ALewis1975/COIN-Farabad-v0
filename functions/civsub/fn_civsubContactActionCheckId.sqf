...
// Guard: pre-compile IdentitySet if not yet resolved (dispatcher does not pre-compile it)
...
// Backfill serial for legacy identity records touched before GenerateProfile populated it
if (_serial isEqualTo "") then {
    ...
    _serial = format ["FRB-11-%1", str (100000 + (_seed mod 900000))];
