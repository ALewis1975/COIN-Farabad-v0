...
_hist = _hist select {
    if !(_x isEqualType [] && { (count _x) >= 5 }) exitWith { false };
    private _ts = _x select 0;
    (_ts isEqualType 0) && { _ts >= _windowStart }
};
...
private _districtCount = count (_hist select {
    if !(_x isEqualType [] && { (count _x) >= 2 }) exitWith { false };
    private _histDistrictId = _x select 1;
    (_histDistrictId isEqualType "") && { (toUpper _histDistrictId) isEqualTo _districtId }
});
...
if (_pos isEqualTo []) then
{
    private _bundlePos = [_bundle, "pos", []] call _hg;
