...
case "FOLLOWON_REQUEST":
    {
        ...
        {
            if (([_holdIntent] call _trimFn) != "") then { _extra pushBack format ["Hold intent: %1", [_holdIntent] call _trimFn]; };
            if (_holdMin > 0) then { _extra pushBack format ["Hold duration: %1 min", _holdMin]; };
        };

        if (_req isEqualTo "PROCEED") then
        {
            if (([_proceedIntent] call _trimFn) != "") then { _extra pushBack format ["Proceed intent: %1", [_proceedIntent] call _trimFn]; };
        };

        if (([_rat] call _trimFn) != "") then { _extra pushBack format ["Rationale: %1", [_rat] call _trimFn]; };
        if (([_con] call _trimFn) != "") then { _extra pushBack format ["Constraints: %1", [_con] call _trimFn]; };
        if (([_sup] call _trimFn) != "") then { _extra pushBack format ["Support: %1", [_sup] call _trimFn]; };
        ...
        if (_stU isEqualTo "APPROVED") then
        {
            ...
            private _ordTxt = "Order: (not yet visible / already completed)";
            {
                if (_x isEqualType [] && { (count _x) >= 7 }) then
                {
                    ...
                    {
                        _ordTxt = format ["Order: %1 (%2) Type: %3 Target: %4", _oid, toUpper _ost, ([_otype] call ARC_fnc_intelOrderTypeLabel), _tgt];
                    };
                };
            } forEach _orders;
