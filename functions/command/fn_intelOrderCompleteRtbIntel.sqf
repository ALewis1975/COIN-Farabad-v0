...
if (!([_caller, "ARC_fnc_intelOrderCompleteRtbIntel", "Request rejected: sender verification failed.", "INTELORDERCOMPLETERTBINTEL_SECURITY_DENIED", false, _reoOwner] call ARC_fnc_rpcValidateSender)) exitWith {false};
...
private _selectionAllowed = if (_orderIdO != "") then
{
    ...
    if (_purposeO != "INTEL") exitWith {false};
    true
}
else
{
