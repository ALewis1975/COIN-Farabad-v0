...
private _fnUnitPlayBlocking = {
    ...
    // Engine state is owned by the caller: helicopter keeps engine on for hover-taxi lift;
    // fixed-wing callers suppress engine before calling this function to prevent AI thrust
    // from fighting BIS_fnc_unitPlay position commands (see taxi suppression block below).
...
// Fixed-wing (including UAV/RPAS): engine thrust generated by a running engine fights
// BIS_fnc_unitPlay's per-frame position commands, accelerating the aircraft toward
// takeoff speed and causing the AI to skip the taxi path. Suppress by:
//   1) turning the engine off so no thrust is produced during scripted playback;
//   2) setting land "GEAR" so the vehicle AI stays in ground mode and does not
//      attempt an autonomous takeoff sequence.
// Helicopters keep their engine on because hover-taxi requires rotor lift.
// Engine is restored immediately after taxi at line below (_veh engineOn true).
if (!_isHeli && { _veh isKindOf "Air" }) then {
    _veh engineOn false;
    _veh land "GEAR";
};
