...
if (_opsLogEnabled || _debugOps) then {
    ["OPS", format ["AIRBASE RUNWAY: %1 -> RESERVED (%2 %3 %4)", _runwayPrev, _fid, _kind, _detail], _center, 0, [
        ["flightId", _fid],
        ["kind", _kind],
        ["detail", _detail],
        ["owner", _fid],
...
[_fid, _kind, _detail] spawn {
    ...
    if (_opsEnabledExec || _dbgOpsExec) then {
        ["OPS", format ["AIRBASE RUNWAY: %1 -> OCCUPIED (%2 %3 %4)", _runwayPrevExec, _fid, _kind, _detail], _opsPosExec, 0, [
            ["flightId", _fid],
            ["kind", _kind],
            ["detail", _detail],
            ["owner", _fid],
    ...
    if (_opsEnabledExec || _dbgOpsExec) then {
        ["OPS", format ["AIRBASE RUNWAY: %1 -> OPEN (%2 result=%3)", _runwayPrevRelease, _fid, if (_ok) then {"COMPLETE"} else {"FAILED"}], _opsPosExec, 0, [
            ["flightId", _fid],
