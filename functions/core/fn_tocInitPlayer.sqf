...
// JTAC CAS prefill, SHADOW ISR lead bridge and TNP partnered-ops requests were
// relocated off the player action menu and into the Farabad Console (S2/INTEL tab,
// "FIELD REQUESTS" section). See functions/ui/fn_uiConsoleIntelPaint.sqf and
// functions/ui/fn_uiConsoleClickPrimary.sqf. The underlying client functions and
// their feature flags are unchanged; the console entry closes the dialog first so
// the in-world marking context (laser/cursor target) is valid when the request runs.
