/*
    ARC_fnc_timePolicyGet

    Canonical Time / Tempo Policy read wrapper.

    Mode C intent: preserve the existing dynamic TOD behavior while giving
    consumers a layer-neutral API name. Legacy ARC_fnc_dynamicTodGetPolicy now
    adapts to this function, so current callers keep their old payload shape.
...
private _updatedAt = missionNamespace getVariable ["ARC_dynamic_tod_lastUpdateTs", -1];
...
_out set ["schema", "ARC_timePolicy_v1"];
_out set ["version", [1,0,0]];
_out set ["builtAtServerTime", serverTime];
_out set ["source", "ARC_dynamic_tod"];
_out set ["updatedAt", _updatedAt];

// Legacy-compatible keys. Existing consumers may read these directly.
