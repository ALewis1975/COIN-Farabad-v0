...
if (_ovNids isEqualType [] && { (count _ovNids) > 0 }) then
{
    ...
    private _ovGroups = [];
    {
        ...
        _ovObjs pushBack _o;
        private _g = group _o;
        if (!isNull _g) then { _ovGroups pushBackUnique _g; };
    } forEach _ovNids;

    if ((count _ovObjs) > 0) then
    {
        if (_defer) then
        {
            [_ovObjs, _anchor, _radius, _minDelay, "overlaySpawn"] call ARC_fnc_cleanupRegister;
        }
        else
        {
            { if (!isNull _x && { !isPlayer _x }) then { deleteVehicle _x; }; } forEach _ovObjs;
            { if (!isNull _x) then { deleteGroup _x; }; } forEach _ovGroups;
        };
    };
};
