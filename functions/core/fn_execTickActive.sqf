...
if (_cpEnabled && { _incTypeU isEqualTo "CHECKPOINT" } && { !((toUpper _objKind) isEqualTo "VBIED_VEHICLE") } && { !((toUpper ([_pos] call ARC_fnc_worldGetZoneForPos)) isEqualTo "AIRBASE") }) then
{
    ...
    private _exists = ([_sites, { (_pos distance2D _x) < 80 }] call _findIfFn) >= 0;
...
private _onSite = ([_players, {private _p = _x;
    ([_anchorPosList, { (_p distance2D _x) <= _radius }] call _findIfFn) >= 0}] call _findIfFn) >= 0;
...
if (_execKind isEqualTo "ROUTE_RECON") then
{
    ...
    {
        _onSite = ([_players, { (_x distance2D _sPos) <= _sRad }] call _findIfFn) >= 0;
    };
};
...
if (_execKind isEqualTo "ROUTE_RECON") exitWith
{
    ...
    {
        private _atStart = ([_players, { (_x distance2D _sPos) <= _sRad }] call _findIfFn) >= 0;
        if (_atStart) then
        {
            _startReached = true;
    ...
    {
        private _atEnd = ([_players, { (_x distance2D _ePos) <= _eRad }] call _findIfFn) >= 0;
        if (_atEnd) then
        {
            _endReached = true;
