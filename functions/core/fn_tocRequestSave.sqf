...
if (!isNil "remoteExecutedOwner") then
{
    ...
    if (_reo > 0) then
    {
        ...
        if (!([_requestor, "ARC_fnc_tocRequestSave", "Save rejected: sender verification failed.", "TOC_SAVE_SECURITY_DENIED", true] call ARC_fnc_rpcValidateSender)) exitWith {false};
