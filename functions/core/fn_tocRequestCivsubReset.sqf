...
private _rpcAuthorized = if (isRemoteExecuted || { !isNull _requester }) then
{
    ...
    if (!([_requester, _rpc, "CIVSUB reset rejected: sender verification failed.", "TOC_CIVSUB_RESET_SECURITY_DENIED", true, _reoOwner] call ARC_fnc_rpcValidateSender)) exitWith {false};
...
{
    diag_log format ["[ARC][SEC] ARC_fnc_tocRequestCivsubReset: AUTHORIZATION_DENIED owner=%1 ts=%2", remoteExecutedOwner, serverTime];
    ["ARC_fnc_tocRequestCivsubReset", "AUTHORIZATION_DENIED", remoteExecutedOwner] call ARC_fnc_securityDenyRecord;
    false
};
