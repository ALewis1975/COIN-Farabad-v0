...
if ((count _this) > 4 && { (_this select 3) isEqualType 0 } && { (_this select 4) isEqualType [] }) then
{
    _meta = _this select 4;
};
...
if ((count _posATL) < 2) then { _posATL = [0,0,0]; };
...
if !((_posATL select 0) isEqualTo 0 && {(_posATL select 1) isEqualTo 0}) then
{
    ...
    { if (_x isEqualType [] && { (count _x) > 0 } && { (_x select 0) isEqualTo "grid" }) exitWith { _foundGrid = true; }; } forEach _meta;
    if (!_foundGrid) then
    {
        _meta pushBack ["grid", _grid];
    };
    private _foundZone = false;
    { if (_x isEqualType [] && { (count _x) > 0 } && { (_x select 0) isEqualTo "zone" }) exitWith { _foundZone = true; }; } forEach _meta;
    if (!_foundZone) then
    {
